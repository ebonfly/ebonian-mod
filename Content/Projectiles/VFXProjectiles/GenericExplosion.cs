...
{
    public override string Texture => Helper.AssetPath+"Extras/Fire";
    ...
    {
        if (Projectile.scale > 1.5f && Projectile.ai[1] < 0.1f)
        {
            hitbox.Width = 300;
            hitbox.Height = 300;
...
public class BloodExplosionWSprite : ModProjectile
{
    ...
    public override void OnSpawn(IEntitySource source)
    {
        Helper.AddCameraModifier(new PunchCameraModifier(Projectile.Center, Main.rand.NextVector2Unit(), 3, 6, 30, 1000));

...
public class JuvenileBrain : ModNPC
{
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 pos, Color drawColor)
    {
        ...
        Vector2 orig = new Vector2((float)(Request<Texture2D>(Helper.AssetPath+"NPCs/Crimson/JuvenileBrain").Value.Width / 2), (float)(Request<Texture2D>(Helper.AssetPath+"NPCs/Crimson/JuvenileBrain").Value.Height / Main.npcFrameCount[NPC.type] / 2));
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            Main.spriteBatch.Draw(Request<Texture2D>(Helper.AssetPath+"NPCs/Crimson/JuvenileBrain").Value, new Vector2(startP.X - pos.X + (float)(NPC.width / 2) - (float)Request<Texture2D>(Helper.AssetPath+"NPCs/Crimson/JuvenileBrain").Value.Width * NPC.scale / 2f + orig.X * NPC.scale, startP.Y - pos.Y + (float)NPC.height - (float)Request<Texture2D>(Helper.AssetPath+"NPCs/Crimson/JuvenileBrain").Value.Height * NPC.scale / (float)Main.npcFrameCount[NPC.type] + 4f + orig.Y * NPC.scale + NPC.gfxOffY), new Microsoft.Xna.Framework.Rectangle?(frame6), col * _alpha, NPC.rotation * .1f, orig, NPC.scale, spriteEffects2, 0);

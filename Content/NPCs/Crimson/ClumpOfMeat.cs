    ...
    public override void AI()
    {
        ...
        if (AITimer > 0)
        {
            ...
            if (AITimer > 60)
            {
                ...
                for (int j = 0; j < 10; j++)
                {
                    MPUtils.NewProjectile(null, Main.rand.NextVector2FromRectangle(NPC.getRect()), new Vector2(Main.rand.NextFloat(-10, 10), Main.rand.NextFloat(-7, -4)), ProjectileType<AmbientGibs>(), 0, 0);
                }
                for (int k = 0; k < 5; k++)
                {
                    if (!Main.dedServ)

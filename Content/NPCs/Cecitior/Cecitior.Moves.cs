...
public partial class Cecitior : ModNPC
{
    ...
    public void DoTongue()
    {
        ...
        {
            SoundEngine.PlaySound(Sounds.cecitiorSpit, NPC.Center);
            tongue = MPUtils.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Clamp(Helper.FromAToB(NPC.Center, player.Center), new Vector2(-0.35f, 1), new Vector2(0.35f, 1)) * 1.5f, ProjectileType<LatcherTongueCecitior>(), 15, 0, -1, NPC.whoAmI);
        }
        if (AITimer < 20)

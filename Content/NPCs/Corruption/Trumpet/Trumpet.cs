...
namespace EbonianMod.Content.NPCs.Corruption.Trumpet;

public class TrumpetHead : WormHead
{
    public override string Texture => Helper.AssetPath + "NPCs/Corruption/Trumpet/"+Name;
    public override void SetStaticDefaults()
    {
        NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers()
        {
            CustomTexturePath = Helper.AssetPath+"NPCs/Corruption/Trumpet/TrumpetBestiary",
...
public class TrumpetBody : WormBody
{
    ...
    public override void ExtraAI()
    {
        ...
        if (++NPC.ai[2] % 150 == 50 + NPC.whoAmI * Main.rand.Next(1, 6))
        {
            if (FollowingNPC.type != Type)
            {
                SoundEngine.PlaySound(Sounds.trumpet, NPC.Center);
            }

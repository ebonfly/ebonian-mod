...
namespace EbonianMod.Content.NPCs.Corruption;

public class Glutton : ModNPC
{
    public override string Texture => Helper.AssetPath + "NPCs/Corruption/"+Name;
    ...
    public override void FindFrame(int frameHeight)
    {
        ...
        if (NPC.frameCounter % 5 == 0)
        {
            if (AIState == 0)
            {
                ...
                {
                    if (NPC.frame.Y < 5 * frameHeight && !NPC.velocity.X.InRange(0, 0.1f))
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 pos, Color lightColor)
    {
        ...
        Texture2D glowTexture = Assets.NPCs.Corruption.Glutton_Glow.Value;
        Texture2D bloomTexture = Assets.NPCs.Corruption.Glutton_Bloom.Value;
        Vector2 origin = new Vector2((drawTexture.Width / 2) * 0.5F, (drawTexture.Height / Main.npcFrameCount[NPC.type]) * 0.5F);

        Vector2 drawPos = new Vector2(
            NPC.position.X - pos.X + (NPC.width / 2) - (TextureAssets.Npc[Type].Value.Width / 2) * NPC.scale / 2f + origin.X * NPC.scale,
        ...
        if (eyeBeamAlpha > 0)
        {
            spriteBatch.Draw(bloomTexture, drawPos, NPC.frame, Color.White with { A = 0 } * eyeBeamAlpha, NPC.rotation, origin, NPC.scale, effects, 0);
    ...
    {
        writer.WriteVector2(storedPlayerPos);
    }
    ...
    public override void AI()
    {
        ...
        else if (AIState == Conjure)
        {
            ...
            if (AITimer == 65)
            {
                for (int i = 0; i < 12; i++)
                {
                    ...
                    MPUtils.NewProjectile(null, pos, Helper.FromAToB(pos, player.Center).RotatedBy(-MathHelper.PiOver4 * 0.3f).RotatedByRandom(MathHelper.PiOver4 * 0.5f) * Main.rand.NextFloat(5, 10), ProjectileType<TerrorVilethorn1>(), 30, 0);
                }
        ...
        else
        {
            ...
            if (AITimer == 45)
                SoundEngine.PlaySound(Sounds.BeamWindUp, NPC.Center);
            if (AITimer == 100)
            {
                NPC.netUpdate = true;
                storedPlayerPos = player.Center;
                MPUtils.NewProjectile(null, NPC.Center + new Vector2(56 * NPC.direction, 22), Vector2.Zero, ProjectileType<GreenChargeUp>(), 0, 0);
            }
            if (AITimer == 90)
                MPUtils.NewProjectile(null, NPC.Center + new Vector2(56 * NPC.direction, 22), Vector2.Zero, ProjectileType<GreenChargeUp>(), 0, 0);
            if (AITimer == 155)
                MPUtils.NewProjectile(null, NPC.Center + new Vector2(56 * NPC.direction, 22), Vector2.Zero, ProjectileType<OstertagiExplosion>(), 0, 0);
            if (AITimer > 155 && AITimer <= 174)
            {
                if (AITimer % 2 == 0)
                {
                    ...
                    Helper.AddCameraModifier(new PunchCameraModifier(NPC.Center, Main.rand.NextVector2Unit(), 6, 6, 30, 1000));
                    SoundEngine.PlaySound(Sounds.xSpirit.WithPitchOffset(0.4f).WithVolumeScale(1.2f), NPC.Center);
                    MPUtils.NewProjectile(null, NPC.Center + new Vector2(56 * NPC.direction, 22), Helper.FromAToB(NPC.Center + new Vector2(56 * NPC.direction, 22), storedPlayerPos).RotatedByRandom(MathHelper.PiOver4 * 0.35f) * Main.rand.NextFloat(2, 13), ProjectileType<TFlameThrower>(), 30, 0);
                }

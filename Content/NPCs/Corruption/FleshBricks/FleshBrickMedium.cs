...
namespace EbonianMod.Content.NPCs.Corruption.FleshBricks;

public class FleshBrickMedium : ModNPC
{
    public override string Texture => Helper.AssetPath + "NPCs/Corruption/FleshBricks/FleshBrickMedium0";
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        Texture2D texture2 = Assets.NPCs.Corruption.FleshBricks.FleshBrickMedium1.Value;
    ...
    public override void AI()
    {
        ...
        switch (AIState)
        {
            ...
            case X:
                ...
                if (off == -1)
                {
                    if (player.Center.Y.InRange(NPC.Center.Y, NPC.width * 2))
                    {
                ...
                if (NPC.Center.X.InRange(player.Center.X, NPC.height) && AITimer > 3)
                    AITimer = 31;
                if (AITimer > 30)
                {
                    AIState = Halt;
            ...
            case Y:
                ...
                if (off == -1)
                {
                    if (player.Center.X.InRange(NPC.Center.X, NPC.width * 2))
                    {
                        if (!Collision.CanHit(NPC, player))
                ...
                if (NPC.Center.Y.InRange(player.Center.Y, NPC.width) && AITimer > 3)
                    AITimer = 31;
                if (AITimer > 30)
                {
                    AIState = Halt;

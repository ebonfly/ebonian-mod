...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Magic/IchorGlobScepter";
    ...
    {
        Texture2D texture = Request<Texture2D>(Helper.AssetPath+"Items/Weapons/Magic/IchorGlobScepter_Glow", AssetRequestMode.ImmediateLoad).Value;
        spriteBatch.Draw
        (
            texture,
            new Vector2
...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Magic/IchorGlob";
    public override void SetDefaults()
    {
        Projectile.width = 22;
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        if (ringScale > 0.01f)
        {
            for (float i = 0; i < max; i++)
            {
                ...
                Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.Maroon with { A = 0 } * ringScale, angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.ai[2] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
            }
        }
        ...
        if (ringScale > 0.01f)
        {
            for (float i = 0; i < max; i++)
            {
                ...
                for (float j = 0; j < 2; j++)
                    Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.Maroon with { A = 0 } * (ringScale * 0.7f), angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.ai[1] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
            }
        }
        rand = new UnifiedRandom(seed + 1);
        ...
        if (ringScale > 0.01f)
        {
            for (float i = 0; i < max; i++)
            {
                ...
                for (float j = 0; j < 2; j++)
                    Main.spriteBatch.Draw(tex, Projectile.Center + offset - Main.screenPosition, null, Color.Maroon with { A = 0 } * ringScale, angle, tex.Size() / 2, new Vector2(MathHelper.Clamp(Projectile.ai[0] * 6.5f, 0, 1), ringScale) * scale * 0.2f * 4, SpriteEffects.None, 0);
            }
        }
        return true;
    }
...
{
    public override string Texture => Helper.AssetPath + "Items/Weapons/Magic/IchorGlobSmall";
